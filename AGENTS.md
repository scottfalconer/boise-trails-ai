...
* **Strategy Summary:** The planner will typically cluster nearby segments together into route “clusters” and then compute the shortest loop that covers those segments. It will choose start/end trailheads near each cluster (possibly using known trailheads or safe road endpoints) and ensure a loop or out-and-back is possible. Internally, algorithms like greedy ordering and 2-opt optimization may be used to refine the route and eliminate unnecessary backtracking. The result is a set of daily routes that collectively cover all required trails. This document guides the agent on rules, but the implementation will handle the routing algorithms.
