...
MAX_AUTOMATIC_REPEAT_DISCOVERY_IDS = 20
MAX_AUTOMATIC_REPEAT_DISCOVERY_CUE_MILES = 2.5
MAX_AVOIDABLE_REPEAT_REPAIR_INTERVAL_MILES = 2.5
...
def accepted_candidate_satisfied_by_package(
    ...
    if expected_candidate_id in actual_candidate_ids:
        return True
    if any(
        expected_candidate_id in {str(value) for value in component.get("route_truth_replaces_candidate_ids") or []}
...
def field_menu_source_regression_failures(
    map_data: dict[str, Any],
    replacements_json: Path | None = None,
) -> list[str]:
    return accepted_replacement_regression_failures(map_data, replacements_json)


def validate_field_menu_source(
    map_data: dict[str, Any],
    replacements_json: Path | None = None,
) -> None:
    failures = field_menu_source_regression_failures(map_data, replacements_json)
    if failures:
...
def source_path_route_interval(
    cue: dict[str, Any],
    route_points: list[dict[str, Any]],
    *,
    floor_mile: float,
    ...
    end_point = coordinate_pair(cue.get("source_path_end"))
    if not start_point or not end_point:
        return None
    start_candidates = route_miles_near_point(route_points, tuple(start_point))
    end_candidates = route_miles_near_point(route_points, tuple(end_point))
    if not start_candidates or not end_candidates:
        return None
    floor = max(0.0, float(floor_mile or 0) - floor_slop_miles)
    usable_starts = [float(mile) for mile in start_candidates if float(mile) >= floor]
    if not usable_starts:
        return None
    target_start = max(0.0, float(floor_mile or 0))
    start = min(usable_starts, key=lambda mile: (abs(mile - target_start), mile))
    start = max(target_start, start)
    target_end = start + float(cue.get("leg_miles") or 0)
    usable_ends = [float(mile) for mile in end_candidates if float(mile) > start + 0.005]
    if not usable_ends:
        return None
    end = min(usable_ends, key=lambda mile: (abs(mile - target_end), mile))
    if end <= start:
        return None
    return start, end
...
def official_segment_span_end_miles_for_ids(
    segment_ids: list[Any],
    official_index: dict[str, dict[str, Any]],
    route_points: list[dict[str, Any]],
    *,
    ...
    """Return cue-local completion endpoints for required segments.

    A route can cover a named trail group as a legal edge-cover walk rather than
    as one continuous trail-name sweep. For those cases, the cue should extend
    through the route mile where each required segment span has both endpoints
    represented, even when the official-mile estimate would stop earlier.
    """

    span_ends: list[float] = []
    floor = float(start_mile or 0) + 0.03
    for segment_id in normalized_segment_ids(segment_ids):
        feature = official_index.get(str(segment_id))
        if not feature:
            continue
        feature_span_ends: list[float] = []
        for part in route_parts(feature):
            if len(part) < 2:
                continue
            start_hits = [float(mile) for mile in route_miles_near_point(route_points, part[0]) if float(mile) >= floor]
            end_hits = [float(mile) for mile in route_miles_near_point(route_points, part[-1]) if float(mile) >= floor]
            if start_hits and end_hits:
                feature_span_ends.append(min(max(left, right) for left in start_hits for right in end_hits))
        if feature_span_ends:
            span_ends.append(round(min(feature_span_ends), 3))
    return sorted(set(span_ends))
...
def route_interval_end_for_official_cue(
    ...
    *,
    ...
    if span_ends:
        ...
        span_allowance = max(0.5, float(cue.get("leg_miles") or 0) * 0.15)
        if span_end > start_mile + 0.03 and span_end <= target + span_allowance:
            return span_end
...
def route_off_label_candidate_segment_ids(route: dict[str, Any], official_index: dict[str, dict[str, Any]]) -> set[str]:
    ...
    for cue in route.get("route_cues") or []:
        ...
        link_sources.extend(
            segment.get("pre_connector_link") or {}
...
def turn_phrase_for_transition(
    ...
    points = downsample_coords(flatten_track_segments(track_segments), max_points=500)
...
def route_claim_index(
    routes: list[dict[str, Any]],
    ownership_cards: list[dict[str, Any]] | None = None,
) -> dict[str, list[dict[str, Any]]]:
    claims: dict[str, list[dict[str, Any]]] = {}
    for route in [*routes, *(ownership_cards or [])]:
        outing = route.get("outing") or route
        is_manual_hold = bool(outing.get("manual_design_hold") or route.get("manual_design_hold"))
        claim = {
            "outing_id": outing.get("outing_id") or route.get("outing_id"),
            "label": outing.get("label") or route.get("label"),
            "candidate_ids": [
                str(candidate_id)
                for candidate_id in (outing.get("candidate_ids") or route.get("candidate_ids") or [])
            ],
...
def coordinate_path(path_coordinates: Any) -> list[tuple[float, float]]:
    points: list[tuple[float, float]] = []
    for value in path_coordinates or []:
        pair = coordinate_pair(value)
        if pair:
            points.append((pair[0], pair[1]))
    return points


def path_length_miles(path_coordinates: Any) -> float:
    path = coordinate_path(path_coordinates)
    return sum(haversine_miles(left, right) for left, right in zip(path, path[1:]))


def append_source_path(track: list[tuple[float, float]], path_coordinates: Any) -> None:
    path = coordinate_path(path_coordinates)
    if len(path) < 2:
        return
    ...
    if not track:
        track.extend(path)
        return
    if haversine_miles(track[-1], path[0]) <= 0.005:
        track.extend(path[1:])
        return
    track.extend(path)


def track_segments_for_route_cues(cue_list: list[dict[str, Any]]) -> list[list[tuple[float, float]]]:
    track: list[tuple[float, float]] = []
    for cue in cue_list:
        append_source_path(track, (cue.get("start_access") or {}).get("path_coordinates"))
        groups = trail_groups(cue.get("segments") or [])
        links = list(cue.get("between_links") or [])
        ...
        append_source_path(track, (cue.get("return_to_car") or {}).get("path_coordinates"))
    return [track] if len(track) >= 2 else []


def track_starts_and_ends_near_parking(
    track_segments: list[list[tuple[float, float]]],
    parking: dict[str, Any],
    *,
    max_gap_miles: float,
) -> bool:
    if not track_segments or parking.get("lon") is None or parking.get("lat") is None:
        return False
    parking_point = (float(parking["lon"]), float(parking["lat"]))
    start = first_point(track_segments)
    end = last_point(track_segments)
    return bool(
        start
        and end
        and haversine_miles(parking_point, start) <= max_gap_miles
        and haversine_miles(parking_point, end) <= max_gap_miles
    )


def select_track_segments_for_outing(
    cue_track_segments: list[list[tuple[float, float]]],
    feature_track_segments: list[list[tuple[float, float]]],
    parking: dict[str, Any],
    *,
    ...
    if track_starts_and_ends_near_parking(
        ...
        parking,
        max_gap_miles=max_parking_gap_miles,
    ):
        candidates.append(("route_feature", feature_track_segments, 0))
    if track_starts_and_ends_near_parking(
        cue_track_segments,
        parking,
        max_gap_miles=max_parking_gap_miles,
    ):
        candidates.append(("route_cues", cue_track_segments, 1))
    if cue_track_with_parking_connectors and track_starts_and_ends_near_parking(
        cue_track_with_parking_connectors,
        parking,
        max_gap_miles=max_parking_gap_miles,
    ):
        candidates.append(("route_cues_parking_repaired", cue_track_with_parking_connectors, 2))
    if candidates:
        if prefer_route_cues:
            for source, track_segments, _priority in candidates:
                if source == "route_cues":
                    return track_segments, source
        if walkthrough_graph:
            graph_score_cache: dict[int, tuple[int, int, int]] = {}

            def graph_scores(track_segments: list[list[tuple[float, float]]]) -> tuple[int, int, int]:
                cache_key = id(track_segments)
                if cache_key in graph_score_cache:
                    return graph_score_cache[cache_key]
                if not track_segments:
                    graph_score_cache[cache_key] = (10_000, 10_000, 10_000)
                    return graph_score_cache[cache_key]
                sampled = resample_track_segments_for_matching(track_segments, spacing_miles=0.05)
                groups, unmatched = matched_edge_groups(sampled, walkthrough_graph, 0.015)
                blocked_count = len(
                    {
                        str(edge.edge_id)
                        for group in groups
                ...
                graph_score_cache[cache_key] = (blocked_count, direction_violation_count, len(unmatched))
                return graph_score_cache[cache_key]
...
def track_with_parking_connectors(
    ...
    *,
    ...
    if len(track_segments) != 1 or not connector_graph:
        return track_segments
    if parking.get("lon") is None or parking.get("lat") is None:
        return track_segments
    track = list(track_segments[0])
    if len(track) < 2:
        return track_segments
    parking_point = (float(parking["lon"]), float(parking["lat"]))
    start = track[0]
    if haversine_miles(parking_point, start) > max_parking_gap_miles:
        mapped = shortest_connector_path(parking_point, start, connector_graph, snap_tolerance_miles)
        if mapped:
            prefix = coordinate_path(path_with_original_endpoints(parking_point, mapped.get("path_coordinates") or [], start))
            if prefix:
                track = prefix + track[1:]
    end = track[-1]
    if haversine_miles(end, parking_point) > max_parking_gap_miles:
        mapped = shortest_connector_path(end, parking_point, connector_graph, snap_tolerance_miles)
        if mapped:
            suffix = coordinate_path(path_with_original_endpoints(end, mapped.get("path_coordinates") or [], parking_point))
            if suffix:
                track.extend(suffix[1:])
    return [track] if len(track) >= 2 else track_segments


def link_endpoint_pair(link: dict[str, Any]) -> tuple[tuple[float, float], tuple[float, float]] | None:
    start = coordinate_pair(link.get("path_start"))
    end = coordinate_pair(link.get("path_end"))
...
def link_distance_miles(link: dict[str, Any]) -> float:
    for key in ("distance_miles", "leg_miles"):
        value = link.get(key)
        if value is not None:
            return float(value or 0)
    return (
        float(link.get("connector_miles") or 0)
        + float(link.get("official_repeat_miles") or 0)
        + float(link.get("road_miles") or 0)
    )


def rounded_miles(value: Any) -> float:
    try:
...
def path_with_original_endpoints(
    start: tuple[float, float],
    mapped_path_coordinates: Any,
    end: tuple[float, float],
) -> list[list[float]]:
    points: list[tuple[float, float]] = [start]
    points.extend(coordinate_path(mapped_path_coordinates))
    points.append(end)
    deduped: list[list[float]] = []
    for point in points:
        if deduped and haversine_miles((deduped[-1][0], deduped[-1][1]), point) <= 0.001:
            continue
...
def apply_shortest_connector_path_to_link(
    ...
    *,
    ...
    path_miles = path_length_miles(link.get("path_coordinates"))
    missing_connector_proof = bool(path_miles > 0 and not link.get("connector_edges"))
    if path_miles > 0 and (current_miles <= 0 or missing_connector_proof):
        current_miles = max(current_miles, path_miles)
    max_search_miles = (
        current_miles + unsafe_repair_extra_tolerance_miles
        if current_miles > 0
        else None
    )
    mapped = shortest_connector_path(
        ...
        max_distance_miles=max_search_miles,
    )
    if not mapped:
        return False
    shortest_miles = float(mapped.get("distance_miles") or 0)
    if current_miles <= 0:
        return False
    ...
    if missing_connector_proof and shortest_miles - current_miles > 0.1:
        return False
    original_source = link.get("source")
    link.update(
        {
            "source": "shortest_connector_repair",
            "source_before_shortest_repair": original_source,
            "distance_miles": rounded_miles(shortest_miles),
            "connector_miles": rounded_miles(mapped.get("connector_miles") or 0),
            "official_repeat_miles": rounded_miles(mapped.get("official_repeat_miles") or 0),
            "official_repeat_segment_ids": normalized_segment_ids(mapped.get("official_repeat_segment_ids") or []),
            "connector_names": unique_nonempty_text(mapped.get("connector_names") or []),
            "signpost_labels": signpost_labels(mapped.get("connector_names") or []),
            "connector_classes": unique_nonempty_text(mapped.get("connector_classes") or []),
            "connector_edges": mapped.get("connector_edges") or [],
            "path_coordinates": path_with_original_endpoints(start, mapped.get("path_coordinates") or [], end),
            "path_start": [start[0], start[1]],
    ...
    if "road_miles" in link:
        link["road_miles"] = 0
    return True


def apply_shortest_connector_repairs(
    ...
    *,
    ...
    for cue in cue_list:
        for link in cue.get("between_links") or []:
            avoid_ids = {
                int(seg_id)
                for seg_id in link.get("avoided_unearned_segment_ids") or []
                if str(seg_id).isdigit()
            }
            if apply_shortest_connector_path_to_link(
                link,
                connector_graph,
...
def cue_source_path(cue: dict[str, Any]) -> list[tuple[float, float]]:
    path = coordinate_path(cue.get("source_path_coordinates"))
    return path if len(path) >= 2 else []


def cue_declared_leg_miles(cue: dict[str, Any]) -> float:
    candidates = []
    for key in ("leg_miles", "source_leg_miles", "route_leg_miles"):
        value = cue.get(key)
        if value is not None:
            candidates.append(float(value or 0))
    source_path_miles = path_length_miles(cue.get("source_path_coordinates"))
    if source_path_miles > 0:
        candidates.append(source_path_miles)
    return max(candidates, default=0.0)


def wayfinding_cue_interval_path(
    cue: dict[str, Any],
    route_points: list[dict[str, Any]],
) -> list[tuple[float, float]]:
    source_path = cue_source_path(cue)
    if source_path:
        return source_path
    interval = cue_route_interval(cue)
    if not interval and cue.get("route_miles") is not None and float(cue.get("leg_miles") or 0) > 0:
        start = float(cue.get("route_miles") or 0)
        interval = (start, start + float(cue.get("leg_miles") or 0))
    if not interval and cue.get("cum_miles") is not None and float(cue.get("leg_miles") or 0) > 0:
        start = float(cue.get("cum_miles") or 0)
        interval = (start, start + float(cue.get("leg_miles") or 0))
    if not interval:
        return []
    return route_interval_coordinates(route_points, start_mile=interval[0], end_mile=interval[1])


def apply_shortest_repairs_to_wayfinding_cues(
    route: dict[str, Any],
    connector_graph: dict[str, Any] | None,
    *,
    snap_tolerance_miles: float = 0.045,
    ...
    cues = route.get("wayfinding_cues") or []
    route_points = cumulative_track_points(route.get("_track_segments") or [])
    required_ids = set(normalized_segment_ids(route.get("segment_ids") or (route.get("outing") or {}).get("remaining_segment_ids") or []))
    ...
    for cue in cues:
        ...
        if credited_ids and cue_type in NON_CREDIT_WAYFINDING_CUE_TYPES:
            ...
            avoid_ids = {
                int(seg_id)
                for seg_id in (required_ids - credited_ids)
                if str(seg_id).isdigit()
            }
            if len(path) >= 2 and current_miles > 0:
                start = path[0]
                end = path[-1]
                mapped = shortest_connector_path(
                    start,
                ...
                if mapped:
                    shortest_miles = float(mapped.get("distance_miles") or 0)
                    if current_miles - shortest_miles > improvement_tolerance_miles:
                        ...
                        cue["source_path_start"] = source_path[0]
                        cue["source_path_end"] = source_path[-1]
                        cue["official_repeat_segment_ids"] = repeat_ids
                        cue["official_repeat_miles"] = repeat_miles
...
def trail_groups(segments: list[dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    for segment in segments:
        ...
        pre_connector_link = segment.get("pre_connector_link")
        if groups and groups[-1]["trail_name"] == trail_name and not pre_connector_link:
            ...
            groups.append(
                {
                    ...
                    "segments": [segment],
                    "incoming_link": pre_connector_link,
                }
            )
    return groups
...
def make_wayfinding_cue(
    *,
    ...
    official_miles: Any = None,
    official_repeat_segment_ids: list[Any] | None = None,
    official_repeat_miles: Any = None,
    source_path_start: Any = None,
    source_path_end: Any = None,
    source_path_coordinates: Any = None,
) -> dict[str, Any]:
    ...
    cue = {
        ...
        "official_miles": round(float(official_miles or 0), 4),
        "official_repeat_segment_ids": repeat_segment_ids,
        "official_repeat_miles": repeat_miles,
        "source_path_start": coordinate_pair(source_path_start),
        "source_path_end": coordinate_pair(source_path_end),
        "source_path_coordinates": coordinate_path(source_path_coordinates),
    }
...
def full_claimed_segments_in_cue_interval(
    ...
    *,
    ...
    if len(interval_coords) < 2:
        return set()
    review_coords = downsample_coords(interval_coords, max_points=max_activity_points)
    interval_bbox = coordinate_bbox(interval_coords)
    bbox_buffer = max(0.003, threshold_miles / 15.0)
    endpoint_prefilter_miles = max(0.08, threshold_miles * 2)
    ...
    for review_index in (official_index, default_official_feature_index()):
        ...
        for segment_id in claimed_segment_ids:
            ...
            if not feature:
                continue
            if interval_bbox and not feature_near_bbox(feature, interval_bbox, buffer_degrees=bbox_buffer):
                continue
            if not feature_endpoints_near_path(
                feature,
                review_coords,
                threshold_miles=endpoint_prefilter_miles,
            ):
                continue
...
def apply_non_credit_claimed_repeat_declarations(
    ...
    *,
    ...
    credited_claimed_ids: set[str] = set()
    for cue in cues:
        ...
        added_ids = (completed_claimed_ids & credited_claimed_ids) - existing_ids
...
def alternate_path_for_repeated_cue(
    *,
    ...
    if current_miles > MAX_AVOIDABLE_REPEAT_REPAIR_INTERVAL_MILES:
        return None
    max_alternate_distance = current_miles - post_credit_repeat_savings_threshold(current_miles)
    if max_alternate_distance <= 0:
        return None
    ...
    alternate = shortest_connector_path(
        ...
        snap_tolerance_miles,
        avoid_official_segment_ids=avoid_ids,
        max_distance_miles=max_alternate_distance,
    )
    if not alternate:
        return None
    path_coords = [
...
def coordinate_bbox(coords: list[tuple[float, float]] | list[list[float]]) -> tuple[float, float, float, float] | None:
...
def bboxes_overlap_with_buffer(
    left: tuple[float, float, float, float],
    right: tuple[float, float, float, float],
    *,
    ...
    return not (
        left[2] + buffer_degrees < right[0]
        or right[2] + buffer_degrees < left[0]
        or left[3] + buffer_degrees < right[1]
        or right[3] + buffer_degrees < left[1]
    )


def feature_near_bbox(
    feature: dict[str, Any],
    ...
    *,
    ...
    for part in route_parts(feature):
        part_bbox = coordinate_bbox(part)
        if part_bbox and bboxes_overlap_with_buffer(bbox, part_bbox, buffer_degrees=buffer_degrees):
            return True
    return False


def feature_endpoints_near_path(
    feature: dict[str, Any],
    coords: list[tuple[float, float]],
    *,
    threshold_miles: float,
    ...
    for part in route_parts(feature):
        if len(part) < 2:
            continue
        if (
            point_to_polyline_distance_miles(part[0], coords) <= threshold_miles
            and point_to_polyline_distance_miles(part[-1], coords) <= threshold_miles
        ):
            return True
    return False
...
def route_has_intentional_route_truth_lollipop_repeats(route: dict[str, Any]) -> bool:
    ...
    if route_quality.get("field_shape") == "lower_dry_access_shingle_up_dry_creek_down":
        return True
    return any(
        cue.get("cue_generation_mode") == "route_truth_repair_explicit_lollipop"
        for cue in route.get("route_cues") or []
    )
    ...
    *,
    ...
    for segment_id in segment_ids:
        ...
        near_count = sum(
            1
            for point in official_points
            if point_to_polyline_distance_miles(point, track_points) <= threshold_miles
        )
        if near_count / len(official_points) < min_fraction:
            missing.add(str(segment_id))
    return missing
...
def replacement_preserves_claimed_endpoint_coverage(
    ...
    *,
    ...
    return not official_endpoint_miss_ids(
        repaired,
        claimed_segment_ids,
        review_index,
    ) and not official_geometry_miss_ids(
        repaired,
        claimed_segment_ids,
        review_index,
    )
...
def repair_avoidable_post_credit_repeats(
    ...
    *,
    ...
    if not connector_graph:
        return []
    if route_has_intentional_route_truth_lollipop_repeats(route):
        route["intentional_post_credit_repeat_policy"] = "route_truth_lollipop"
        return []
    ...
    for iteration in range(1, 7):
        ...
        if after_miles >= before_miles - 0.01:
            break
        route_savings = before_miles - after_miles
        if route_savings < max(0.75, before_miles * 0.10):
            break
...
def access_wayfinding_cue(
    *,
    ...
    source_path_start = access.get("path_start")
    source_path_end = access.get("path_end")
    if hint:
        return make_wayfinding_cue(
            ...
            source_path_end=source_path_end,
            source_path_coordinates=access.get("path_coordinates"),
        )
...
def link_wayfinding_cue(
    *,
    ...
    return make_wayfinding_cue(
        ...
        official_repeat_miles=official_repeat_miles,
        source_path_start=link.get("path_start"),
        source_path_end=link.get("path_end"),
        source_path_coordinates=link.get("path_coordinates"),
    )
...
def return_wayfinding_cue(
    *,
    ...
    if (
        str(return_to_car.get("strategy") or "") == "closed_loop"
        and float(return_to_car.get("connector_miles") or 0) <= 0
        and float(return_to_car.get("road_miles") or 0) <= 0
        and float(return_to_car.get("official_repeat_miles") or 0) <= 0
    ):
        access_miles = 0.0
    else:
        access_miles = max(
            float(return_access_gap_miles or 0),
            float(return_to_car.get("distance_miles") or 0),
            float(return_to_car.get("connector_miles") or 0),
            float(return_to_car.get("road_miles") or 0),
    ...
    return make_wayfinding_cue(
        ...
        official_repeat_miles=official_repeat_miles,
        source_path_start=return_to_car.get("path_start"),
        source_path_end=return_to_car.get("path_end"),
        source_path_coordinates=return_to_car.get("path_coordinates"),
    )
...
def primary_field_visible_names(names: list[Any]) -> list[str]:
    clean_names = unique_nonempty_text(names)
    if not clean_names:
        return []
...
def first_wayfinding_names_for_types(
    cues: list[dict[str, Any]],
    cue_types: set[str],
    *,
    reverse: bool = False,
) -> list[str]:
    ordered_cues = list(reversed(cues)) if reverse else cues
    for cue in ordered_cues:
        if str(cue.get("cue_type") or "") not in cue_types:
            continue
        names = primary_field_visible_names(cue.get("signed_as") or [])
        if names:
            return names
    return []


def append_missing_names_to_step(step: dict[str, str], names: list[str], prefix: str) -> None:
    step_text = normalized_trail_text(
        f"{step.get('title') or ''} {step.get('detail') or ''}"
    ).lower()
    missing = [
        name
        for name in names
        if normalized_trail_text(name).lower() not in step_text
    ]
    append_names_to_step(step, missing, prefix)


def synchronize_turn_steps_with_wayfinding_cues(route: dict[str, Any]) -> dict[str, Any]:
    cues = route.get("wayfinding_cues") or []
    steps = route.get("turn_by_turn_steps") or []
    if not cues or not steps:
        return route
    access_names = first_wayfinding_names_for_types(cues, {"start_access"})
    return_names = first_wayfinding_names_for_types(
        cues,
        {"exit_access", "return_to_car"},
        reverse=True,
    )
    for step in steps:
        kind = str(step.get("kind") or "").lower()
        if kind == "access" and access_names:
            append_missing_names_to_step(step, access_names, "Access also follows")
        elif kind == "return" and return_names:
            append_missing_names_to_step(step, return_names, "Return also follows")
    return route
...
def enrich_route_with_walkthrough_edge_names(
    ...
    *,
    ...
    if not track_segments or not graph_edges or not route.get("wayfinding_cues"):
        return route
    source_edges = graph_edges.edges if isinstance(graph_edges, TrailGraph) else graph_edges
    local_edges = filter_edges_for_track(source_edges, track_segments)
    if not local_edges:
        return route
    graph = TrailGraph(local_edges)
    all_text = ""
    preferred_text = " ".join(cue_text_values(route))
...
def route_navigation_source_failures(route: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    for cue in route.get("wayfinding_cues") or []:
        ...
        if (
            route_has_intentional_route_truth_lollipop_repeats(route)
            and cue.get("official_repeat_segment_ids")
            and map_miles > 0.05
        ):
            continue
        ...
        if crossed_car_passes and map_miles > max((cue_miles * 2.0) + 0.25, cue_miles + 1.0):
            failures.append(
                {
            ...
            continue
        diff_miles = abs(map_miles - cue_miles)
        ratio = max(map_miles, cue_miles) / max(min(map_miles, cue_miles), 0.01)
        if diff_miles < 0.75 or ratio < 1.25:
            continue
        failures.append(
            {
                "code": "cue_map_mileage_mismatch",
                "cue_seq": cue.get("seq"),
                "cue_type": cue.get("cue_type"),
                "cue_miles": round(cue_miles, 3),
                "map_miles": round(map_miles, 3),
                "difference_miles": round(diff_miles, 3),
                "ratio": round(ratio, 3),
                "route_miles_start": round(start_miles, 3),
                "route_miles_end": round(end_miles, 3),
                "signed_as": cue.get("signed_as") or [],
...
def split_wayfinding_cues_at_car_passes(route: dict[str, Any]) -> None:
    ...
    for cue in cues:
        ...
        for boundary in interior_passes + [end]:
            ...
            part["leg_miles"] = round(max(0.01, boundary - part_start), 2)
...
<body>
  ...
  <script>
    ...
    function sourcePathSegmentsForCue(cue) {
      const coords = Array.isArray(cue?.source_path_coordinates) ? cue.source_path_coordinates : [];
      if (!coords.length || !state.project) return [];
      const points = coords
        .map(coord => Array.isArray(coord) && coord.length >= 2 ? state.project({ lon: Number(coord[0]), lat: Number(coord[1]) }) : null)
        .filter(point => point && Number.isFinite(point.x) && Number.isFinite(point.y));
      return points.length > 1 ? [points] : [];
    }
    function mergeRouteMRanges(ranges, maxGapM = 55) {
      const sorted = ranges
        .filter(range => Number.isFinite(range.startM) && Number.isFinite(range.endM) && range.endM > range.startM)
        .sort((a, b) => a.startM - b.startM);
      const merged = [];
      for (const range of sorted) {
        const current = merged[merged.length - 1];
        if (current && range.startM <= current.endM + maxGapM) {
          current.endM = Math.max(current.endM, range.endM);
        } else {
          merged.push({ ...range });
        }
      }
    ...
    function routeSampleAt(routeM) {
      const point = positionForRouteM(routeM);
      const angle = tangentForRouteM(routeM);
    ...
    function selfOverlapRangesForRouteRange(startM, endM) {
      const span = Math.max(0, endM - startM);
      if (span < 120) return [];
      const sampleStepM = 15;
      const minRouteGapM = 75;
      const overlapDistanceM = Math.max(16, Math.min(26, currentSchematicLaneSpacingM() * 1.2));
      const samples = [];
      for (let routeM = startM; routeM <= endM; routeM += sampleStepM) {
        const sample = routeSampleAt(routeM);
      ...
      for (let leftIndex = 0; leftIndex < samples.length; leftIndex += 1) {
        ...
        for (let rightIndex = leftIndex + 1; rightIndex < samples.length; rightIndex += 1) {
          ...
          if ((right.routeM || 0) - (left.routeM || 0) < minRouteGapM) continue;
          if (distance(left, right) > overlapDistanceM) continue;
          if (angleDifference(left.angle, right.angle) < 2.2) continue;
          ranges.push(
            { startM: Math.max(startM, (left.routeM || 0) - sampleStepM), endM: Math.min(endM, (left.routeM || 0) + sampleStepM) },
            { startM: Math.max(startM, (right.routeM || 0) - sampleStepM), endM: Math.min(endM, (right.routeM || 0) + sampleStepM) }
          );
        }
      }
      return mergeRouteMRanges(ranges, 60).filter(range => range.endM - range.startM >= 45);
    }
    function activeSelfOverlapRanges() {
      const leg = activeLegRange();
      return selfOverlapRangesForRouteRange(leg.startM, leg.endM);
    }
    function subtractActiveSelfOverlapRanges(startM, endM) {
      let ranges = [{ startM, endM }];
      for (const repeat of activeSelfOverlapRanges()) {
        const nextRanges = [];
    ...
    function nonActiveSelfOverlapSegmentsForRouteRange(startM, endM, options = {}) {
      const output = [];
      for (const range of subtractActiveSelfOverlapRanges(startM, endM)) {
        output.push(...segmentsForRouteRange(range.startM, range.endM, options));
      }
      return output;
    }
    function activeSelfOverlapTrunkSegmentsForRouteRange(startM, endM) {
      const output = [];
      for (const range of activeSelfOverlapRanges()) {
    ...
    function drawProgressRibbon(startAtM = 0) {
      ...
      for (let startM = firstM; startM < state.totalRouteM; startM += stepM) {
        ...
        const chunkSegments = nonActiveSelfOverlapSegmentsForRouteRange(startM, endM, { context: true });
    ...
    function drawRoute() {
      ...
      const visibleSegments = nonActiveSelfOverlapSegmentsForRouteRange(visibleStartM, state.totalRouteM, { context: true });
      ...
      if (state.style === "cue-legs") {
        ...
        for (let index = 1; index < stops.length; index += 1) {
          const legSegments = nonActiveSelfOverlapSegmentsForRouteRange(stops[index - 1], stops[index], { context: true });
      ...
      const leg = activeLegRange();
      const sourceActiveSegments = sourcePathSegmentsForCue(leg.cue);
      const activeSegments = smoothSegmentsForDisplay(
        sourceActiveSegments.length
          ? sourceActiveSegments
      ...
      if (!sourceActiveSegments.length) {
...
def route_field_tool_record(route: dict[str, Any], completion_safety: dict[str, Any] | None = None) -> dict[str, Any]:
    ...
    return {
        ...
        "source_card_on_foot_miles": outing.get("source_card_on_foot_miles"),
        "route_quality": route.get("route_quality") or outing.get("route_quality") or {},
...
def build_field_tool_data(
    ...
    payload = {
        ...
        "summary": {
            ...
            "held_route_count": len(manifest.get("manual_holds") or []),
...
def export_field_packet(
    ...
    field_menu_replacements_json: Path | None = None,
) -> dict[str, Any]:
    ...
    validate_field_menu_source(map_data, field_menu_replacements_json)
    ...
    walkthrough_track_graph = TrailGraph(walkthrough_graph_edges) if walkthrough_graph_edges else None
    ...
    for outing in sorted(
        ...
        feature_track_segments = track_segments_for_outing(outing, routes_by_candidate)
        raw_track_segments, track_source = select_track_segments_for_outing(
            cue_track_segments,
            feature_track_segments,
            parking,
            max_parking_gap_miles=max_parking_gap_miles,
        ...
        route = {
            ...
            "shortest_connector_repair_count": shortest_repair_count,
            "source_gap_repair": source_gap_repair,
            "route_quality": outing.get("route_quality") or {},
        ...
        if walkthrough_graph is not None and not route_has_intentional_route_truth_lollipop_repeats(route):
        ...
        if late_repeat_repairs:
            ...
            apply_off_label_route_leg_warnings(route)
            route["segment_direction_evidence"] = segment_direction_evidence_for_route(route)
            if not route_has_intentional_route_truth_lollipop_repeats(route):
    ...
    manifest = {
        "summary": {
            ...
            "held_route_count": len(manual_holds),
...
def main() -> int:
    ...
    try:
        manifest = export_field_packet(
            ...
            field_menu_replacements_json=args.field_menu_replacements_json,
        )
