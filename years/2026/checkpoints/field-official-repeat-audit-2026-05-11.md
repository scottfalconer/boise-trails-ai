# Field Official Repeat Audit

Generated: 2026-05-12T17:48:00Z
Status: `passed`

## Summary

- Routes audited: 48
- Source repeat legs: 99
- Public repeat cues: 105
- Bucket A bad hidden self-repeat: 0
- Bucket B counted/cued repeats or optimization targets: 99
- Bucket C reconciled extra-credit routes: 24
- Repeat legs missing segment IDs: 0
- Repeat cues missing repeat/no-credit text: 0
- Unreconciled extra-credit segments: 0

## Top Non-Credit Burdens

| Label | Candidate | Official mi | On-foot mi | Non-credit mi |
|---|---|---:|---:|---:|
| FD30A | `harlows-hollows-connector-ricochet-shooting-range-twisted-spring` | 2.61 | 13.62 | 11.01 |
| FD24A | `harlows-hollows` | 1.40 | 11.81 | 10.41 |
| FD04A | `combo-two-point-femrites-patrol-shanes-connector` | 1.70 | 9.55 | 7.85 |
| FD20A | `three-bears-trail-freestone-ridge` | 6.72 | 13.10 | 6.38 |
| 18 | `block-bogus_mores_lodge_tempest` | 5.08 | 11.25 | 6.17 |
| 16A-1 | `manual-16a-1` | 6.09 | 12.20 | 6.11 |
| FD06A | `fat-tire-traverse-curlew-connection` | 4.09 | 8.59 | 4.50 |
| 3 | `block-military_core` | 8.31 | 12.13 | 3.82 |
| FD26A | `around-the-mountain-trail` | 6.64 | 10.17 | 3.53 |
| FD25A | `elk-meadows-trail` | 1.49 | 4.90 | 3.41 |

## Bucket A

- None.

## Bucket C

- `FD09A` `barn-owl` reconciles extra official segment ids: ['1624']
- `18` `block-bogus_mores_lodge_tempest` reconciles extra official segment ids: ['1553', '1750']
- `3` `block-military_core` reconciles extra official segment ids: ['1748']
- `12` `block-upper_8th_corrals_sidewinder` reconciles extra official segment ids: ['1498', '1532']
- `14` `block-watchman_five_mile_rocky` reconciles extra official segment ids: ['1558', '1685']
- `4A` `bobs-trail-urban-connector` reconciles extra official segment ids: ['1577']
- `FD14B` `chbh-connector` reconciles extra official segment ids: ['1541', '1599', '1604']
- `FD03A` `chukar-butte-trail` reconciles extra official segment ids: ['1624']
- `FD04A` `combo-two-point-femrites-patrol-shanes-connector` reconciles extra official segment ids: ['1593', '1594', '1629', '1630', '1631', '1649', '1650', '1651', '1683', '1684', '1695']
- `FD22B` `crestline-trail` reconciles extra official segment ids: ['1615']
- `FD14A` `doe-ridge` reconciles extra official segment ids: ['1599', '1604']
- `FD25A` `elk-meadows-trail` reconciles extra official segment ids: ['1655']
- `FD06A` `fat-tire-traverse-curlew-connection` reconciles extra official segment ids: ['1483', '1484', '1485', '1486', '1564', '1751']
- `FD24A` `harlows-hollows` reconciles extra official segment ids: ['1708']
- `FD30A` `harlows-hollows-connector-ricochet-shooting-range-twisted-spring` reconciles extra official segment ids: ['1661', '1696', '1706']
- `FD05A` `hulls-gulch-interpretive` reconciles extra official segment ids: ['1483']
- `FD19B` `lower-hulls-gulch-trail-red-cliffs` reconciles extra official segment ids: ['1484', '1532', '1533']
- `10B` `manual-10b` reconciles extra official segment ids: ['1520', '1521', '1619', '1624']
- `16A-2` `manual-16a-2` reconciles extra official segment ids: ['1542', '1543']
- `FD19C` `shanes-trail` reconciles extra official segment ids: ['1593', '1594', '1629', '1630', '1631', '1683', '1748']
- `FD27A` `single-segment-1661-spring-creek::Avimor Spring Valley Creek parking::Avimor Spring Valley Creek parking` reconciles extra official segment ids: ['1687', '1688', '1689']
- `FD27B` `single-segment-1662-spring-creek::Avimor Spring Valley Creek parking::Avimor Spring Valley Creek parking` reconciles extra official segment ids: ['1661', '1687', '1688', '1689', '1706']
- `FD25B` `the-face-trail` reconciles extra official segment ids: ['1679']
- `FD20A` `three-bears-trail-freestone-ridge` reconciles extra official segment ids: ['1593', '1594', '1629']
